...
def plot_tsne(FE, Classifier, src_dl, tgt_dl, device, save_dir, model_type,
               train_mode, run_id):  # , layer_output_to_plot, y_test, save_dir, type_id):
    ...
    with torch.no_grad():
        ...
        src_features, _ = FE(src_data)
        src_predictions = Classifier(src_features)
        ...
        tgt_features, _ = FE(tgt_data)
        tgt_predictions = Classifier(tgt_features)
    ...
    file_name = "tsne_" + model_type + "_" + train_mode + "_" + f"{run_id}" + ".png"
    ...
    file_name = "tsne_" + model_type + "_" + train_mode + "_domain-based" + "_" + f"{run_id}" + ".png"
    fig_save_name = os.path.join(save_dir, "tsne_plots", file_name)
    plt.savefig(fig_save_name)
    # wandb.log({f"{file_name}": wandb.Image(plt)})
    plt.close()
